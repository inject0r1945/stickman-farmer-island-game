...
namespace HSVStudio.Tutorial
{
    ...
    public class HSVTutorialManager : MonoBehaviour
    {
        ...
        private void InitializeTutorialObjectsForStageDirect(HSVTutorialStage stageObject)
        {
            //Sets the current play step to first one
            stageObject.currentStep = 1;
            //Reset playing state to idle
        ...
        /// </summary>
        public void StopStage(bool autoAdvance = true)
        {
            if (!Application.isPlaying || !tmInitialized)
                return;
            ...
            if (currentPlayingStage.state == PlayState.Start || currentPlayingStage.state == PlayState.Executing)
            {
                ...
                StartCoroutine(StopStageCoroutine(autoAdvance));
            }
            else
            {
        ...
        /// <returns></returns>
        private IEnumerator StopStageCoroutine(bool autoAdvance)
        {
            ...
            if (currentStageStep < stageObjects.Length)
            {
                ...
                if (autoAdvance && addStartTrigger)
                {
        ...
        public void ResetStageDirect(HSVTutorialStage stageObj)
        {
            if (stageObj != null)
            {
                ResetStageObjectDirect(stageObj);
            }
        }
        ...
        private void ResetStageObjectDirect(HSVTutorialStage stageObj)
        {
            InitializeTutorialObjectsForStageDirect(stageObj);

            DebugLog("Complete Resetting stage: " + stageObj.stageName + " index: " + stageObj.index);
        }

        public void ReplayStage(HSVTutorialStage stageObj)
        {
            if (stageObj != null)
            {
                StartCoroutine(ReplayStageCoroutine(stageObj));
            }
        }

        private IEnumerator ReplayStageCoroutine(HSVTutorialStage stageObj)
        {
            if(currentPlayingStage != null)
            {
                StopStage(false);
            ...
            while(currentPlayingStage != null)
            {
                yield return null;
            }

            ResetStageDirect(stageObj);
        ...
        public void ClearTargetTrigger(HSVTarget target)
        {
            if (target.rtTarget != null)
            {
                ...
                if (target.triggerConfig != null)
                {
                    ...
                    {
                        case TriggerType.Collider:
                            if (target.triggerConfig.colliderConfig != null && target.triggerConfig.colliderConfig.collider != null)
                            {
                                configObj = target.triggerConfig.colliderConfig.collider.transform;
                            }
                            break;
                        case TriggerType.UI:
                            if (target.triggerConfig.graphicConfig != null && target.triggerConfig.graphicConfig.graphic != null)
                            {
        ...
        public void UpdateElementIndex(int selectIndex)
        {
            ...
            if (stageObj != null)
            {
                for (int i = 0; i < stageObj.tutorialObjects.Length; i++)
                {
                    if (stageObj.tutorialObjects != null && stageObj.tutorialObjects[i].index != i + 1)
                    {
                        stageObj.tutorialObjects[i].index = i + 1;
                    }
                    if (stageObj.tutorialObjects != null && stageObj.tutorialObjects[i].stageIndex != selectIndex + 1)
                    {
